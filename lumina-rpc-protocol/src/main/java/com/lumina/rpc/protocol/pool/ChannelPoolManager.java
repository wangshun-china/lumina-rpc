...
public class ChannelPoolManager {
     ...
     * 获取指定地址的池状态
     */
    public String getPoolStatus(InetSocketAddress address) {
        String addressKey = address.getHostString() + ":" + address.getPort();
        ChannelPool pool = poolMap.get(addressKey);
        return pool != null ? pool.toString() : "No pool for " + addressKey;
    }
    ...
    public String getAllPoolStatus() {
        ...
        sb.append("ChannelPoolManager Status:\n");
        sb.append("  Global channels: ").append(globalChannelCount.get()).append("/").append(globalMaxChannels).append("\n");
    ...
    public void shutdown() {
        logger.info("🛑 Shutting down ChannelPoolManager...");
        ...
        logger.info("✅ ChannelPoolManager shutdown complete");
    }

    /**
     * 获取全局连接数
     */
    public int getGlobalChannelCount() {
        return globalChannelCount.get();
    }
